...
- **Confirmed working on a real device (Diego, Google Pixel 9a, 2026-08-27):** the generated PNG downloads correctly and looks right. On Firefox for Android, sharing offers a download instead of the native share sheet — that's correct, expected fallback behaviour, not a bug: Firefox doesn't implement `navigator.share` at all, on any platform, so there's no code-side fix for it (confirmed this explicitly with Diego, who then tried Chrome). On **Chrome for Android, the real native share sheet works** — confirmed sharing straight to WhatsApp. Diego asked about adding a small "saved — open WhatsApp and attach it" message for the Firefox/download-fallback path; declined for now.

### 8. History link from the running fast — tried and reverted

**Status:** not pursued — reverted per Diego's feedback same day
**Requested by:** Diego, 2026-08-27

Diego asked for a way to reach History from the active-fast screen. Built it as an explicit "history" link next to "settings" in the eyebrow row (`feature/history-link-during-fast`, briefly on the dev server) — Diego didn't like it: "The recent fasts below is enough" (`RecentFasts`, the "recent 16h02 15h48…" row at the bottom of the active-fast card, already links to `#/history`). Branch discarded, not merged. **Don't re-add an explicit eyebrow history link** — `RecentFasts` is the intended path there, even though it only renders once `history.length > 0` (no link at all for a first fast in progress) — that gap was raised but explicitly not worth fixing this way.