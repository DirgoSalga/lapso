...
- Left the fill **transparent** (no background) — the page's own progress-driven background wash (spec §5.2, written per-frame onto `document.body`) already carries colour here; a second solid card fill would fight it rather than sit quietly, per the app's "everything else stays quiet" design language.
