...
- **The card itself** (`FastCard`/`FastRingBadge` in `History.tsx`): a static SVG ring (not the live `<Ring>` component, which is built around a per-frame animation loop and an imperative handle a finished record has no use for), coloured via the same `ringColor()` the live ring and the card-glow feature already use, showing `min(duration/goal, 1)` — plus duration, goal, the exact start→end range, and the note if any, all reusing existing formatters. Deliberately has **no** glow/backdrop treatment — that visual language (feature #4) signals "a fast is happening right now" elsewhere in the app, which would misrepresent a finished record. A `#/history/<id>` that doesn't resolve to any fast falls back to the list rather than a dead end.